...
2. Unfold the top most tree element (latest stable Qt version, e.g. `Qt 5.15.3`)
...
To produce a standalone package, you need to generate all required files using the tool `windeployqt`. This tool can be found in the `bin` directory of your Qt installation, e.g. at `C:\Qt\5.15.3\msvc2019_64\bin\windeployqt.exe`.

To produce all supplement files for a standalone build, follow these steps (adjust paths as required):

1.  Navigate to your build output directory with Windows Explorer, e.g. `C:\Users\example\src\build-chatterino-Desktop_Qt_5_15_3_MSVC2019_64bit-Release`
...
4.  Open a command prompt and execute:
        ...
        cd C:\Users\example\src\build-chatterino-Desktop_Qt_5_15_3_MSVC2019_64bit-Release\release
        C:\Qt\5.15.3\msvc2019_64\bin\windeployqt.exe chatterino.exe
...
```
-DCMAKE_PREFIX_PATH=C:\Qt\5.15.3\msvc2019_64\lib\cmake\Qt5
-DUSE_CONAN=ON
...
- Set the _Program_ to `C:\Qt\5.15.3\msvc2019_64\bin\windeployqt.exe`
- Set the _Arguments_
